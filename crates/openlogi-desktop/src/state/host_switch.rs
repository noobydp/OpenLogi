...
impl AppState {
    ...
    pub(crate) fn set_host_switch_target_enabled(
        ...
        let keyboard = self.current_record()?;
        let keyboard_key = keyboard.persistent_config_key()?.to_string();
        ...
        (changed && self.persist_and_reload("Easy-Switch follower")).then_some(event_key)
    }
...
mod tests {
    ...
    use super::{
        super::ConfigPersistence, AppState, DeviceRecord, is_compatible_target, set_target_enabled,
    };
    ...
    fn selecting_a_compatible_follower_persists_identity_and_rejects_self_links() {
        ...
        assert!(
            state
                .set_host_switch_target_enabled("unit:05060708", true)
                .is_some()
        );
        assert_eq!(
        ...
        assert!(
            state
                .set_host_switch_target_enabled("unit:01020304", true)
                .is_none()
        );
        assert_eq!(
            state.config.devices["unit:01020304"].host_switch_targets,
        ...
        assert!(
            state
                .set_host_switch_target_enabled("unit:090a0b0c", true)
                .is_none()
        );
        assert!(receiver.try_recv().is_err());
    }
